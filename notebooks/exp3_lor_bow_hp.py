...
# Filter to 'happiness' and 'sadness'
...
# MLflow Experiment
# -------------------------------
mlflow.set_experiment("LoR_Hyperparameter_Tuning")  # Experiment name
...
# -------------------------------
with mlflow.start_run(run_name="LR_GridSearch_Parent") as parent_run:
    ...
    # Log each param combination as nested run
    for params, mean_score, std_score in zip(
        ...
        with mlflow.start_run(run_name=f"LR_Params_{params}", nested=True):
            ...
            print(f"✅ LR_Params_{params} -> F1: {f1:.4f}, Accuracy: {accuracy:.4f}")
    ...
    # Save & Log Best Model
    # -------------------------------
    model_dir = "best_lr_model"
    if os.path.exists(model_dir):
        shutil.rmtree(model_dir)

    mlflow.sklearn.save_model(grid_search.best_estimator_, model_dir)
    ...
    print("✅ Hyperparameter tuning, best model & notebook logged successfully 🚀")
